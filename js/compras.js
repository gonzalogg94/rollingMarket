...
window.eliminarProducto = function (codigo) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      let copiaListaCompras = listaCompras.filter((compra) => {
        return compra.codigoBarra != codigo;
      });
      listaCompras = copiaListaCompras;
      guardarDatosLS();
      actualizarTabla();
      Swal.fire("Listo!", "El carrito se actualizó", "success");
    }
  });
};
...
window.sumatoria = function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      Swal.fire('<a href="/pages/error404.html">Link de Pago</a>');
    }
  });
};
